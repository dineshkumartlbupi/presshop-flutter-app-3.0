...
public class SwiftFacebookAppEventsPlugin: NSObject, FlutterPlugin {
    ...
    public func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey : Any] = [:]) -> Bool {
        // FBSDKCoreKit.ApplicationDelegate.shared.application(application, didFinishLaunchingWithOptions: launchOptions as [UIApplication.LaunchOptionsKey: Any]?)
        return true
    }
    ...
    private func handleGetApplicationId(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        // FIXME: Settings.shared.appID not found in FBSDK 18.0.2?
        result(nil) 
    }
    ...
    private func handleSetAutoLogAppEventsEnabled(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        FBSDKCoreKit.Settings.shared.isAutoLogAppEventsEnabled = enabled
        result(nil)
    }
    ...
    private func handleSetDataProcessingOptions(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        // FIXME: setDataProcessingOptions not found in FBSDK 18.0.2?
        // FBSDKCoreKit.Settings.shared.setDataProcessingOptions(modes, country: country, state: state)
        // print("setDataProcessingOptions ignored due to compilation error")
        result(nil) 
    }
