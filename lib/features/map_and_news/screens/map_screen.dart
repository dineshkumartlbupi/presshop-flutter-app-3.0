...
import 'package:geolocator/geolocator.dart'; // Added here
...
import 'package:presshop/view/map_and_news/widgets/custom_info_window.dart';
import 'package:presshop/view/map_and_news/widgets/danger_zone_info_window.dart';
import 'package:presshop/view/map_and_news/widgets/serarch_filter_widget.dart';
import 'package:presshop/view/map_and_news/widgets/side_action_panal.dart';
...
class _MarketplaceScreenState extends ConsumerState<MarketplaceScreen>
  ...
  Future<void> _initMap() async {
    ...
    var myLocation = await mapController.mapService.getCurrentLocation();

    if (myLocation == null && mounted) {
      debugPrint("Location service unavailable. Showing permission dialog.");
  ...
  void _showLocationPermissionDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "This app needs location access to show nearby events and alerts. "
          "Please enable location services or grant permission in settings.",
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> _useDefaultLocation() async {
    final mapController = ref.read(mapControllerProvider.notifier);
    // Default to London
    final defaultLocation = const LatLng(51.5074, -0.1278);
    await mapController.setMyLocation(defaultLocation);

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Showing default location (London)."),
  ...
  Future<void> _selectPlace(String placeId, String description) async {
    ...
    if (response.statusCode == 200) {
      ...
      ref.read(mapControllerProvider.notifier).setSearchedLocation(latLng);
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(mapControllerProvider, (previous, next) {
      if (previous?.selectedPosition != next.selectedPosition ||
        ...
        if (next.selectedPosition != null &&
            previous?.selectedPosition != next.selectedPosition) {
          _controller.future.then((ctrl) {
            ctrl.animateCamera(CameraUpdate.newLatLng(next.selectedPosition!));
          });
        }
      ...
      body: Stack(
        children: [
          ...
          if (_infoOffset != null && state.selectedIncident != null)
            Positioned(
              left: _infoOffset!.dx -
                  (state.selectedIncident!.markerType == 'content' ? 32 : 140),
              top: _infoOffset!.dy -
                  (state.selectedIncident!.markerType == 'content' ? 260 : 195),
          ...
          if (_routeInfoOffset != null && state.routeInfo != null)
            Positioned(
              left: _routeInfoOffset!.dx - 75,
              top: _routeInfoOffset!.dy - 60 - 12,
              child: Column(
                children: [
                  Container(
                    width: 150,
                  ...
                  child: AlertPanel(
                    ...
                    onAlertSelected: (type) async {
                      try {
                        ...
                        // mapController.toggleAlertPanel(); // Handled by AlertPanel
                        if (state.myLocation != null) {
                          ...
                          debugPrint("AlertSelected: Location not available");
                        }
                      } catch (e) {
