...
class LeadershipTableWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print('memberList in LeadershipTableWidget====> $memberList');
    print(
        'memberList length in LeadershipTableWidget====> ${memberList.length}');

    Size size = MediaQuery.of(context).size;
    List<Member?> safeMembers = List<Member?>.from(memberList);
    while (safeMembers.length < 3) {
      safeMembers.add(null); // fill missing slots with null (dummy)
    }
